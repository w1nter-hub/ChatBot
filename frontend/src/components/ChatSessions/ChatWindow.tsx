            ...
            >
                ...
                <Box textAlign="center">
                    <Heading
                        ...
                        textAlign="center"
                    >
                        Клиенттермен чат тарихы осы жерде көрсетіледі.
                     </Heading>
                </Box>
            </VStack>
        )
    ...
    return (
        <Box w="calc(100% - 450px)" h="100%" position="relative">
            ...
            {chatData && isManualChat && (permissions.get().isOwner || permissions.get().isAdmin || permissions.get().isEditor) ? <Box className="chat-input-wrap" style={{ position: 'absolute' }} w={'100%'} bottom={0} bgColor={'white'} overflow={'hidden'} >
                <TextareaAutosize ref={inputRef} autoFocus value={question} onChange={handleChatChange} onKeyDown={handleKeyDown} rows={1} className="chat-input textarea js-auto-size" id="chat-input" placeholder="Хабарламаңызды енгізіңіз" />
