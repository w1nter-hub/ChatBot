...
const EditChatbot = (props: EditChatbotProps) => {
	...
	const rawDefaultStep = history.location.pathname.split('/').pop();
	const defaultStep = (rawDefaultStep === "integrations" ? "analytics" : rawDefaultStep) as Steps;
	...
	const [primaryButtonLabel, setPrimaryButtonLabel] = React.useState<string>("Сайт деректерін жаңарту");
	const [productSetupTab, setProductSetupTab] = React.useState<number>(0);

	const handleTabChange = React.useCallback((tabIndex: number) => {
		if(tabIndex === 0) {
			setPrimaryButtonLabel("Сайт деректерін жаңарту");
			setProductSetupTab(0);
		} else {
			setPrimaryButtonLabel("Файл жүктеу");
	...
	const handleChatbotUpdate = React.useCallback(async (formValues)=>{
		...
		try {
			...
			toast({
				title: (errorData && errorData[0]) || 'Қате орын алды',
				status: "error",
				isClosable: true,
			});
		}
		...
		async function fetchData() {
			try {
				...
				if(chatBotData.status === 'EMBEDDING_ERROR' || chatBotData.status === 'CRAWL_ERROR') {
					...
					setPrimaryButtonLabel("Чат-бот құру")
...
console.log(permissions.get(), 'permissionspermissions')
	...
	const getAddToWebsiteContent = React.useCallback(() => {
		...
		return <VStack spacing={12} alignItems="start" p={2}>
			...
			<VStack alignItems="start">
				<Heading fontSize="md">Ендірілген чат интерфейсі (лаунчерсіз)</Heading>
				<Text color="gray.500" fontSize="sm">Чат интерфейсі веб-бетте лаунчер белгішесінсіз тұрақты түрде көрсетіледі.</Text>
				<Box className={styles.codeBlock}>
			...
			<VStack alignItems="start">
				<Heading fontSize="md">Чат интерфейсіне ашық сілтеме</Heading>
			...
			>
				...
				<Box textAlign="center">
					<Heading
						...
						textAlign="center"
					>
						Дерек неғұрлым көп болса, чат-бот соғұрлым сапалы жауап береді.
					</Heading>
				</Box>
			</VStack>
		}
	...
	const getDefaultCustomizationValues = React.useCallback(() => {
		...
		const normalizedPrompt = (!chatBot.prompt ||
			chatBot.prompt.startsWith("You are a very enthusiastic chatbot") ||
			chatBot.prompt.includes("Your name is {{chatbotName}}"))
			? chatWidgetDefaultValues.prompt
			: chatBot.prompt;
		const normalizedDefaultAnswer = (!chatBot.defaultAnswer ||
			chatBot.defaultAnswer === "I don't know how to answer that")
			? chatWidgetDefaultValues.defaultAnswer
			: chatBot.defaultAnswer;
		...
		return {
			...
			defaultAnswer: normalizedDefaultAnswer,
		...
		return (
			<>
                            ...
                            defaultCustomizationValues={getDefaultCustomizationValues()}
														...
														onNextClick={async (formData: ChatBotCustomizeData) => {
															try {
																...
																if (formData.model !== chatBot.model) {
				...
				{access.isOwner || access.isAdmin || access.isEditor ?<Flex
					...
					h="100%" overflow="auto"
				>
					<SectionTitle title="Сайтқа қосу" description="Чат-ботты сайтыңызға қосу үшін төмендегі нұсқалардың бірін таңдаңыз." />
				...
				>
					<VStack alignItems="start" w="100%" position="relative">
						<SectionTitle title="Деректерді толықтыру" description="Қосымша оқыту дерегін енгізіп, чат-бот сапасын арттырыңыз. Деректерді логикалық топтармен және қысқа түрде қосыңыз." />
				...
				{currentStep === "chat-sessions" ? <Flex
					...
					h="100%" overflow="auto"
				>
					<SectionTitle title="Чат сессиялары" description="Клиенттермен барлық чат сессиялары." />
					<Flex w="100%" className={styles.trainingDataCont}>
				...
				<Flex
					...
					>
					<SectionTitle
						title="Жеке домен орнату"
						description="Чат-ботқа өз доменіңіз арқылы кіру"
					/>
				...
				<Flex
					direction="column"
					style={{
						display: currentStep === "analytics" ? "flex" : "none",
					}}
					h="100%"
					overflow="auto"
				>
					<SectionTitle
						title="Аналитика"
						description="Қолдау автоматтандыру KPI панелі: AI шешу үлесі, офлайн өтініштер және білім базасы қамтуы."
					/>
					{chatBot._id ? (
						<Analytics
							chatbotId={chatBot._id}
							crawledPages={defaultCrauledData?.stats?.crawledPages || 0}
							failedPages={defaultCrauledData?.stats?.failedPages || 0}
						/>
					) : null}
				</Flex>
				{access.isOwner ? <Flex
				...
				>
					<SectionTitle
						title="Қатысушылар"
						description="Осы чат-ботқа қолжетімділігі бар қолданушыларды басқару"
					/>
				{chatBot._id ? (
					<Members
				...
				<Flex
					...
					h="100%" overflow="auto"
				>
					<SectionTitle title="Офлайн хабарламалар" description="Клиенттер жіберген офлайн хабарламалар." />
					<Flex w="100%" className={styles.trainingDataCont}>
				...
				{access.isOwner || access.isAdmin || access.isEditor ?<Flex
					...
					h="100%" overflow="auto"
				>
					<SectionTitle title="Чат-ботты тексеру" description="Чат-ботпен сөйлесіп, оның жауаптарын тексеріңіз. Нәтиже күткендей болмаса, төмендегі кеңестерді қолданыңыз." />
					<HStack w="100%">
						<Box w="50%">
						...
						<Box w="50%" pos="relative" h="100%">
							<Flex className={styles.resultTips} direction="column">
								<Alert status='info' flexShrink={0} borderRadius="md">
									...
									<Box>
										<AlertTitle fontWeight="600">Дұрыс жауап шықпай жатыр ма?</AlertTitle>
										<AlertDescription fontSize="14px" color="#004c77">
											Бұл толықтай оқыту деректеріне байланысты. Төменде чат-бот дәлдігін арттыру жолдары берілген.
										</AlertDescription>
									</Box>
								</Alert>
								<Box className={styles.resultTip}>
									<Box className={styles.resultTipNumber} > 1</Box>
									<Heading className={styles.resultTipHeading} fontSize="medium">Қосымша оқыту деректерін енгізіңіз</Heading>
									<Text className={styles.resultTipDescg}>Дерек неғұрлым көп болса, чат-бот соғұрлым сапалы жауап береді. Сұрақ-жауап форматында арнайы деректерді <Text as="span" cursor="pointer" textDecoration="underline" onClick={() => {
										goToStep("train-custom-data");
									}}>Арнайы деректерді оқыту</Text> бөлімінен қоса аласыз.</Text>
								</Box>
								<Box className={styles.resultTip}>
									<Box className={styles.resultTipNumber} > 2</Box>
									<Heading className={styles.resultTipHeading} fontSize="medium">Сайт беттерінің дұрыс сканерленгенін тексеріңіз</Heading>
									<Text className={styles.resultTipDescg}>Сайттағы барлық маңызды беттер дұрыс сканерленгенін тексеріңіз. Ол үшін <Text as="span" cursor="pointer" textDecoration="underline" onClick={() => {
										goToStep("product-setup");
									}}>Дерек көздері</Text> бөліміне өтіп, "Сканерленген деректі көру" батырмасын басыңыз.</Text>
								</Box>
								<Box className={styles.resultTip}>
									<Box className={styles.resultTipNumber} > 3</Box>
									<Heading className={styles.resultTipHeading} fontSize="medium">Алдыңғы қолдау сұрақтарымен оқытыңыз</Heading>
									<Text className={styles.resultTipDescg}>Қайталанатын өтініштер мен хаттарды азайту үшін чат-ботты бұрынғы қолдау сұрақтары мен жауаптары арқылы оқытыңыз.</Text>
								</Box>
								<Box className={styles.resultTip}>
									<Box className={styles.resultTipNumber} > 4</Box>
									<Heading className={styles.resultTipHeading} fontSize="medium">Бізбен байланыс</Heading>
									<Text className={styles.resultTipDescg}>Нәтиже көңіліңізден шықпаса, QoldauAI қолдау тобына хабарласыңыз. Орнату бойынша көмектесеміз.</Text>
								</Box>
							</Flex>
						</Box>
					</HStack>
			...
			<Flex shrink={0} alignItems="center" w="100%" justifyContent="start" h="48px" bg="blue.500" color="white" pl="5">
				<Link to="/app/chat-bots/">
					<Flex alignItems="center">
						...
						<Heading ml="2" color="white" fontSize="14" fontWeight="400">Чат-боттар</Heading>
					</Flex>
				</Link>
			</Flex>
			{
					...
					>
						...
						<List spacing={2}>
							{access.isOwner || access.isAdmin || access.isEditor ? <ListItem
								...
								className={currentStep === "analytics" ? styles.active : ""}
							>
								<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
									<path d="M3 3V21H21M7 15L10.5 11.5L13.5 14.5L20 8" stroke="currentcolor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
								</svg>
								Аналитика
							</ListItem> : null}
							...
							>
								...
								Қатысушылар <Badge ml="8px" variant='outline' colorScheme='yellow'>
											Beta
										</Badge>
							</ListItem> : null}
