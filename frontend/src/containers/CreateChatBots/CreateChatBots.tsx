...
export const CreateChatBots = () => {
	...
	const [primaryButtonLabel, setPrimaryButtonLabel] = React.useState("Беттерді тексеру");
	...
	const [productSetupLoadingText, setProductSetupLoadingText] = React.useState("Көмекші бапталуда");
	...
	const handlePrimaryButtonClick = React.useCallback(async (payLoad: ProductSetupData) => {
		setIsSubmitting(true);
			setProductSetupLoadingText('Қолдау білім базасы жасалуда...');
		...
		try {
			...
			if (payLoad.files?.length && payLoad.files.length > 0) {
				...
				for (const file of payLoad.files) {
					try {
						...
						toast({
							title: `${file.name} файлын жүктеу сәтсіз аяқталды. ${(error?.response?.data?.message) || ''}`,
							status: "error",
							isClosable: true,
						});
					}
			...
			let interval = setInterval(async () => {
				...
				if(details.data.websiteData !== null) {
					setProductSetupLoadingText('Сайттан қолдау контенті жиналуда. Бұл біраз уақыт алуы мүмкін...');
				}
				...
				if (details.data.status === 'CRAWLED' || (details.data.status === 'CREATED' && _docsData && _docsData.docs.length > 0)) {
					setProductSetupLoadingText('Көмекші сіздің деректеріңізбен үйретілуде. Бұл біраз уақыт алуы мүмкін...');
					...
					setIsSubmitting(false);
					toast({
						title: `Қолдау чат-боты сәтті құрылды`,
						status: "success",
						isClosable: true,
					});
	...
	const getProductSetupComponent = React.useCallback(() => {
		return (
			<ChatBotProductSetup
				...
				secondaryBtnLabel={"Жолдарды жаңарту"}
		...
		<VStack w="100%" h="100vh" overflow="hidden">
			<Flex shrink={0} alignItems="center" w="100%" justifyContent="start" h="60px" bg="blue.500" color="white" pl="6">
				<Link to="/app/chat-bots/">
					...
					<Heading ml="2" color="white" fontSize="18" fontWeight="400">Қолдау боттары</Heading>
					</Flex>
				</Link>
			</Flex>
